...
// ser-Agent: Mozilla/4.0 (compatible; MSIE5.01; Windows NT)\n\
// ost: www.google.com\n\
...
// \r\n\
// r\n";
            // std::istringstream ss(__dataGatherer);
            // std::string buffer;
...
std::ostream &			operator<<( std::ostream & o, Request const & req )
{
	...
	o << req.getUri() << " ";
	o << req.getProtocol() << " \n";
	for (std::map<std::string, std::string>::iterator i = req.getHeaders().begin(); i != req.getHeaders().end(); i++){
